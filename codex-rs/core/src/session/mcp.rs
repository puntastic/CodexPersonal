...
async fn review_guardian_mcp_elicitation(
        ...
        )
    {
        ...
        return Ok(Some(mcp_elicitation_response_from_guardian_decision(
            decision,
            step_settings.model_info.as_ref(),
        )));
    }
    ...
    if matches!(
        ...
        if !turn_context
            ...
            || crate::connectors::mcp_approvals_reviewer_from_layers(
                ...
                Some(step_settings.model_info.slug.as_str()),
        ...
        let review_context = crate::guardian::GuardianReviewContext::from_resolved_settings(
            Arc::clone(&turn_context),
            step_settings.as_ref(),
        );
        ...
        let crate::guardian::GuardianApprovalOutcome::Decision(decision) = decision else {
            return Ok(None);
        };
    ...
    let mut review_context = crate::guardian::GuardianReviewContext::from_resolved_settings(
        Arc::clone(&turn_context),
        step_settings.as_ref(),
    );
