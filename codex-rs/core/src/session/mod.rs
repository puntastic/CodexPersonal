...
impl Session {
    ...
    async fn record_initial_history(
        ...
        let turn_context = match conversation_history {
            ...
            InitialHistory::Forked(mut rollout_items) => {
                ...
                // succeeds.
                let (pending_fork_history, pending_fork_guardian_history) = {
                    let mut state = self.state.lock().await;
                    (
                        state.take_persisted_history_items(),
                        state.take_persisted_guardian_history_items(),
                    )
                };
                ...
                {
                    let mut state = self.state.lock().await;
                    state.replace_persisted_history_items(durable_items);
                    state.replace_persisted_guardian_history_items(durable_guardian_items);
                }
    ...
    async fn apply_rollout_reconstruction(
        ...
        let persisted_guardian_items = guardian_history
            .as_ref()
            .map(|checkpoint| {
                codex_history::CompactedHistoryResolver::filter_exact_guardian_sources(
                    rollout_items,
            ...
            .unwrap_or_default();
        {
            ...
            state.replace_persisted_guardian_history_items(persisted_guardian_items);
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        };
        {
            ...
            // The model policy declined automatic review for this category.
            // Continue through the existing user permission request below.
        }
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        // Encode only after attaching the model hash so integrity-bound entries
        // cover the exact checkpoint that will be reconstructed.
        ...
        let mut carried_guardian_reference_sources = Vec::new();
        let mut inline_guardian_sources = Vec::new();
        ...
        let mut world_state_item = None;
        {
            ...
            compacted_item.guardian_history =
                state
                    .history
                    .guardian_history_checkpoint()
                    .map(|checkpoint| {
                        let checkpoint = compacted_history::encode_guardian_history(
                            checkpoint,
                            state.persisted_guardian_history_items(),
                            state.session_configuration.history_mode,
                        );
                            ...
                            for entry in entries {
                                match entry {
                                    CompactedHistoryEntry::Inline { item, metadata } => {
                                        inline_guardian_sources.push(ResponseItemEnvelope {
                                    ...
                                    | CompactedHistoryEntry::ReferenceV2 { item_id, .. } => {
                                        if let Some(source) =
                                            state.persisted_guardian_history_items().get(item_id)
                                        {
                                            carried_guardian_reference_sources.push(source.clone());
                                        }
                                    }
                                }
                            }
                        } else {
                            inline_guardian_sources.extend(
                                checkpoint.0.iter().cloned().map(ResponseItemEnvelope::new),
                            );
                        }
                        checkpoint
                    });
        ...
        if self.try_persist_rollout_items(&rollout_items).await {
            // Keep only sources still reachable from the bounded model or Guardian histories.
            // Referenced Guardian sources precede current checkpoint values so an inline
            // same-ID replacement retains the writer's last-explicit-source semantics.
            carried_guardian_reference_sources.extend(inline_guardian_sources);
            let mut state = self.state.lock().await;
            state.replace_persisted_history_items(persisted_items);
            state.replace_persisted_guardian_history_items(carried_guardian_reference_sources);
        }
        {
            let mut state = self.state.lock().await;
