...
pub(crate) async fn decide_approval(
    ...
    // Full Access is an unconditional bypass and must not read or mutate the lane.
    ...
    let request_turn_id = request
        .request
        .as_ref()
        .map(|request| super::approval_request::guardian_request_turn_id(request, &turn.sub_id))
        .unwrap_or(&turn.sub_id);
    let action_summary = request
        .request
        .as_ref()
        .ok()
        .map(super::approval_request::guardian_assessment_action);
    if let Some(decision) = super::review::reject_if_guardian_approval_lane_closed(
        &session,
        turn,
        request_turn_id,
        action_summary.as_ref(),
    )
    .await
    {
    ...
    match session.services.extensions.decide_approval(&input).await {
        ...
        None | Some(ApprovalDecision::AskUser) if require_fresh_review => {
            Some(runtime.review(GuardianReviewReason::FreshRequired).await)
        }
