...
impl RetainedContext {
    ...
    /// Records that rollback could not correlate every retained answer to its source.
    /// Consumers must not treat the surviving set as a complete authorization history.
    pub fn mark_verified_answers_incomplete(&mut self) {
        self.verified_answers_incomplete = true;
    }

    /// Discards retained facts when rollback crossed a boundary that this ledger cannot map.
    pub fn discard_ambiguous_rollback(&mut self) {
        self.verified_answers.clear();
        self.user_messages.clear();
        self.verified_answers_incomplete = true;
        self.user_messages_incomplete = true;
    }

    /// Rolls back at an exact retained user-message boundary when the caller still has
    /// either its acceptance order or its stable message identity. Returns whether the
    /// boundary was resolved without consulting turn identity, which can be shared by steers.
    pub fn rollback_at_user_message_boundary(
        &mut self,
        first_removed_message_id: Option<&str>,
        acceptance_order: Option<u64>,
    ) -> bool {
        let Some(order) = acceptance_order.or_else(|| {
    ...
    /// Rolls back the newest retained user-message boundaries when compaction has removed
    /// their transport metadata. An incomplete or empty ledger cannot establish a cutoff;
    /// discard potentially stale facts and preserve that uncertainty for downstream review.
    pub fn rollback_latest_user_messages(&mut self, num_messages: usize) {
        if num_messages == 0 {
            return;
        }
        let cutoff_order = if self.user_messages_complete() && !self.user_messages.is_empty() {
            let index = self.user_messages.len().saturating_sub(num_messages);
            self.user_messages[index].order
        } else {
            self.discard_ambiguous_rollback();
            return;
        };
        self.verified_answers
            .retain(|entry| entry.order < cutoff_order);
        self.user_messages
            .retain(|entry| entry.order < cutoff_order);
    }
    ...
    pub fn rollback(
        ...
        if self.rollback_at_user_message_boundary(first_removed_message_id, acceptance_order) {
            return;
        }
