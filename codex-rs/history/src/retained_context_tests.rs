...
#[test]
fn metadata_free_rollback_uses_the_retained_user_message_ledger() {
    let mut context = RetainedContext::default();
        ...
        .enumerate()
    {
        ...
        context.record_user_message(
            ...
            RetainedInputSource::Local(Some(message_order)),
        );
        context.record(&RetainedContextEvent::VerifiedAnswer {
            answer: VerifiedAnswer {
                turn_id: "shared-turn".to_owned(),
    ...
    assert_eq!(
        context
            ...
            .collect::<Vec<_>>(),
        vec!["First instruction.", "answer-0"]
    );

    context.rollback_latest_user_messages(99);
    assert!(context.user_messages_complete());
    assert!(context.verified_answers_complete());
    assert_eq!(context.ordered_entries().count(), 0);
    assert_eq!(context.next_order, 4);
}
...
#[test]
fn metadata_free_rollback_keeps_inherited_order_separate_from_local_order() {
    let mut context = RetainedContext::default();
    for (text, source) in [
        ("Inherited restriction.", RetainedInputSource::Inherited),
        ("Local instruction.", RetainedInputSource::Local(Some(0))),
    ] {
        context.record_user_message(
            RetainedUserMessage {
                turn_id: "shared-turn".to_owned(),
