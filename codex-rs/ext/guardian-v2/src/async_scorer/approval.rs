...
impl GuardianApprovalReviewer {
    ...
    async fn decide_request(
        ...
        let mut policy = guardian_config.as_ref().map_or_else(
            || GuardianPolicy::from_legacy(/*scope*/ None).for_model(Some(model)),
            |config| config.policy_for_model(Some(model)),
        );
        if config
            .config_layer_stack
            .requirements()
        ...
        if mode == GuardianReviewMode::Disabled
